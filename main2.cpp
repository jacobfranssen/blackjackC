...
public:
    ...
    std::string raise_question_user_extra_card(std::string Pos_answer_1,std::string Pos_answer_2,std::string player_name){ // question to user wether player wants another card
        std::string choice_player="--"; // choice player to be filled in by user by default "--"
        std::string choice="-"; // output will be based on input user
    ...
    int players_in_game(){ // check the number of players that do not have a score above 21
        int n_players_in_game=Amount_of_Players; // create a new local variable to determine the number of players
        for(int i = 0; i < Amount_of_Players; i++){ // go trough the number of players and substract 1 if the player has a score higher than 21
